...
import { Library, PlaySquare, FileText, Link as LinkIcon, ExternalLink, BookOpen } from 'lucide-react';
...
const StudentLibrary: React.FC = () => {
  ...
  const getIconInfo = (type: string, link: string) => {
    ...
    if (type === 'PlaySquare' || type === 'video' || lowerLink.includes('youtube.com') || lowerLink.includes('youtu.be')) {
      return {
        icon: <PlaySquare className="w-6 h-6 text-rose-500 drop-shadow-[0_0_8px_rgba(244,63,94,0.5)]" />,
